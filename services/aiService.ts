...
Current date and time (user's local timezone): ${new Date().toLocaleString('en-US', {
    year: 'numeric', month: 'long', day: 'numeric',
    hour: '2-digit', minute: '2-digit', hour12: true,
    weekday: 'long', timeZoneName: 'short'
  })}
Current date (ISO format): ${new Date().toISOString()}
User timezone offset: UTC${new Date().getTimezoneOffset() <= 0 ? '+' : ''}${(-new Date().getTimezoneOffset() / 60).toFixed(1)}

**CRITICAL DATE/TIME PARSING RULES:**
- When creating startDate, use the user's LOCAL time, NOT UTC
- If user says "tomorrow at 2pm", calculate the date in their local timezone
- Return dates in ISO 8601 format WITHOUT the 'Z' suffix (which indicates UTC)
- Example: If it's Nov 25, 2025 in user's timezone and they say "tomorrow at 2pm", return "2025-11-26T14:00:00"
- DO NOT convert to UTC - keep the time exactly as the user specified
